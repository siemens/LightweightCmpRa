...
public class InventoryPluginBase implements InventoryInterface {

    protected CheckAndModifyResult POSITIVE_CHECK_RESULT = new CheckAndModifyResult() {
    ...
    protected CheckAndModifyResult NEGATIVE_CHECK_RESULT = new CheckAndModifyResult() {
    ...
    private ObjectNode parseDnStringToObjectNode(String data) {
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            node.put(keyValue[0], keyValue[1]);
        }
        return node;
    }

    protected ObjectNode createJson(byte[] transactionID,
                              String requesterDn,
                              byte[] certTemplate,
                              String requestedSubjectDn,
                              byte[] pkiMessage) throws JsonProcessingException {
        ObjectMapper objectMapper = new ObjectMapper();

        ObjectNode requestData = objectMapper.createObjectNode();
        requestData.put("transactionID", transactionID);

        ObjectNode requesterDnNode = parseDnStringToObjectNode(requesterDn);
        requestData.putPOJO("requesterDN", requesterDnNode);

        ObjectNode subjectDn = parseDnStringToObjectNode(requestedSubjectDn);
        requestData.putPOJO("requestedSubjectDN", subjectDn);

        requestData.put("certTemplate", certTemplate);

        String encodedPkiMessage = Base64.getEncoder().encodeToString(pkiMessage);
        requestData.put("rawPkiMessage", encodedPkiMessage);
